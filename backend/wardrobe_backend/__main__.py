...
def main():
    ...
    if not s.model_is_allowed: raise SystemExit("Active model must be HelloWorld0204/Classification-StyleWell-model")
    if a.check_hardware:
        try:
    ...
    if a.download_model:
        ...
        Database(s.db_path).set_model_status("stylewell-4b", "installed"); print(f"Downloaded only {s.model_id}"); return
